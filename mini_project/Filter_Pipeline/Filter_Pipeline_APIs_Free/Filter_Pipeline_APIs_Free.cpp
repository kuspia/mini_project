...
struct KdTree //Structure created to implement kdtree data structure for the point cloud 
{
...
int main()
{
    ...
    ifstream infile("C:\\Users\\1kusp\\OneDrive\\Documents\\Run_pro\\real\\3bottle_light_2.pcd"); //To fetch the file from local disk
    ...
    */
    ...
    ofstream outfile("C:\\Users\\1kusp\\OneDrive\\Documents\\Run_pro\\apifreeop\\down.pcd");
    ...
    */
    ...
    while ((cloud.size() > 0.01 * nps) && (cloud.size() != 1) && (chky || chkz)) //We are finding the planes until inliers have atleast 1% of the total size or if desired plane is not detected yet
    {
        ...
        if (fy && chky) // Ground Planes extraction 
        {
            ...
            // storing the 2 pcd files
            ofstream yp("C:\\Users\\1kusp\\OneDrive\\Documents\\Run_pro\\apifreeop\\yp.pcd");
            ofstream yc("C:\\Users\\1kusp\\OneDrive\\Documents\\Run_pro\\apifreeop\\yc.pcd");

            yp << "# .PCD v0.7 - Point Cloud Data file format\nVERSION 0.7\nFIELDS x y z rgb\nSIZE 4 4 4 4\nTYPE F F F U\nCOUNT 1 1 1 1\n";
            yc << "# .PCD v0.7 - Point Cloud Data file format\nVERSION 0.7\nFIELDS x y z rgb\nSIZE 4 4 4 4\nTYPE F F F U\nCOUNT 1 1 1 1\n";
            yp << "WIDTH " << cloudppp.size() << endl;
            yc << "WIDTH " << cloudfff.size() << endl;
            yp << "HEIGHT 1\nVIEWPOINT 0 0 0 1 0 0 0\n";
            yc << "HEIGHT 1\nVIEWPOINT 0 0 0 1 0 0 0\n";
            yp << "POINTS " << cloudppp.size() << endl;
            yc << "POINTS " << cloudfff.size() << endl;
            yp << "DATA ascii\n";
            yc << "DATA ascii\n";


            for (int i = 0; i < cloudppp.size(); i++)
            {

                yp << cloudppp[i].x << " " << cloudppp[i].y << " " << cloudppp[i].z << " " << 16711680 << endl;

            }
            for (int i = 0; i < cloudfff.size(); i++)
            {

                yc << cloudfff[i].x << " " << cloudfff[i].y << " " << cloudfff[i].z << " " << 16711680 << endl;

            }

            // Push back the planes and object cloud to respective point cloud vectors
            cloudy.push_back(cloudfff);
            planey.push_back(cloudppp);
        ...
        if (fz && chkz) // Front Planes extraction 
        {
           ...
           //cout << " The above plane is useful along z axis\n";
            ...
            // storing the 2 pcd files
            ofstream zp("C:\\Users\\1kusp\\OneDrive\\Documents\\Run_pro\\apifreeop\\zp.pcd");
            ofstream zc("C:\\Users\\1kusp\\OneDrive\\Documents\\Run_pro\\apifreeop\\zc.pcd");

            zp << "# .PCD v0.7 - Point Cloud Data file format\nVERSION 0.7\nFIELDS x y z rgb\nSIZE 4 4 4 4\nTYPE F F F U\nCOUNT 1 1 1 1\n";
            zc << "# .PCD v0.7 - Point Cloud Data file format\nVERSION 0.7\nFIELDS x y z rgb\nSIZE 4 4 4 4\nTYPE F F F U\nCOUNT 1 1 1 1\n";
            zp << "WIDTH " << cloudppp.size() << endl;
            zc << "WIDTH " << cloudfff.size() << endl;
            zp << "HEIGHT 1\nVIEWPOINT 0 0 0 1 0 0 0\n";
            zc << "HEIGHT 1\nVIEWPOINT 0 0 0 1 0 0 0\n";
            zp << "POINTS " << cloudppp.size() << endl;
            zc << "POINTS " << cloudfff.size() << endl;
            zp << "DATA ascii\n";
            zc << "DATA ascii\n";
            ...
            for (int i = 0; i < cloudppp.size(); i++)
            {
                ...
                zp << cloudppp[i].x << " " << cloudppp[i].y << " " << cloudppp[i].z << " " << 16711680 << endl;

            }
            for (int i = 0; i < cloudfff.size(); i++)
            {

                zc << cloudfff[i].x << " " << cloudfff[i].y << " " << cloudfff[i].z << " " << 16711680 << endl;

            }

            cloudz.push_back(cloudfff);
            planez.push_back(cloudppp);
            zp.close();
            zc.close();
            idz++;
    ...
    */
    ...
    //Ground Plane object clustering 
    vector<vector<PointXYZ<long double>>> gcl;  // Vector that will push back all objects from the ground plane
    if (idy) // It will make sure ground planes are present as non zero value tell us there was some ground plane 
    {
        // Creation of fstream class object
        ofstream outfile_1;
        outfile_1.open("C:\\Users\\1kusp\\OneDrive\\Documents\\Run_pro\\apifreeop\\gcl.txt");
        ...
        double EPSILON = 0.0004;  // It defines the square of radious of sphere in which we will search the MINIMUM_POINTS criteria to decide a point is core point or not
        int  MINIMUM_POINTS = 4;  // Minimum number of neighbors (data points) within epsilon radius.

        vector<PointXYZ<long double>> cloudg = cloudy[0]; // Copying the plane to the cloudg from vector cloudy
        // Creating the KdTree object for the search method of the extraction
        auto* tree = new KdTree;
        for (int i = 0; i < cloudg.size(); i++)
            tree->insert(cloudg[i], i);
        DBSCAN ds(MINIMUM_POINTS, EPSILON, cloudg, tree); //Running DBSACN on point cloud 
        ds.run();

        // Iterating clusters and stroing according to cluster_id
        ...
        for (auto clust : gcl)
        {
            ofstream clusters_a("C:\\Users\\1kusp\\OneDrive\\Documents\\Run_pro\\apifreeop\\gcl" + to_string(c_idx) + ".pcd");
            //cout << "PointCloud representing the Cluster: " << clust.size() << " data points." << std::endl;
            // storing .pcd files location into a .txt file.
            outfile_1 << "C:\\Users\\1kusp\\OneDrive\\Documents\\Run_pro\\apifreeop\\gcl" << c_idx << ".pcd" << endl;
        ...
        std::cerr << "Total objects at the ground clustered is = " << gcl.size() << std::endl;
        outfile_1.close(); // closing the opened file.
    }

    //Front Plane object clustering 
    vector<vector<PointXYZ<long double>>> fcl;  // Vector that will push back all objects from the front plane
    if (idz) // It will make sure front planes are present as non zero value tell us there was some front plane 
    {
        // Creation of fstream class object
        ofstream outfile_1;
        ...
        vector<PointXYZ<long double>> cloudf = cloudz[0]; // Copying the plane to the cloudf from vector cloudz
        // Creating the KdTree object for the search method of the extraction
        auto* tree = new KdTree;
        ...
        for (auto clust : fcl)
        {
            ofstream clusters_a("C:\\Users\\1kusp\\OneDrive\\Documents\\Run_pro\\apifreeop\\fcl" + to_string(c_idx) + ".pcd");
            //cout << "PointCloud representing the Cluster: " << clust.size() << " data points." << std::endl;
            // storing .pcd files location into a .txt file.
            outfile_1 << "C:\\Users\\1kusp\\OneDrive\\Documents\\Run_pro\\apifreeop\\fcl" << c_idx << ".pcd" << endl;
            //storing the cluster points into .pcd file 
            clusters_a << "# .PCD v0.7 - Point Cloud Data file format\nVERSION 0.7\nFIELDS x y z rgb\nSIZE 4 4 4 4\nTYPE F F F U\nCOUNT 1 1 1 1\n";
            clusters_a << "WIDTH " << clust.size() << endl;
            clusters_a << "HEIGHT 1\nVIEWPOINT 0 0 0 1 0 0 0\n";
        ...
        std::cerr << "Total objects at the front clustered is = " << fcl.size() << std::endl;
        outfile_1.close(); // closing the opened file.
